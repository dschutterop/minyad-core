...
def energy_dashboard_body() -> str:
    ...
    <script>
      const solarMax=5, signedMax=3, nominalKwh=10; let powerUnit='kw'; let last={solar:0,battery:0,grid:0,soc:82,soh:98};
      ...
      async function update(){const d=new Date(); $('clock').textContent=d.toLocaleTimeString([], {hour:'2-digit',minute:'2-digit'}); $('date').textContent=d.toLocaleDateString([], {day:'2-digit',month:'short',year:'numeric'}); let battery={},grid={}; try{[grid,battery]=await Promise.all([fetch('/api/dsmr/status').then(r=>r.ok?r.json():{}),fetch('/api/battery/status').then(r=>r.ok?r.json():{})])}catch(e){}
        ...
        const produced=last.solar*5.8, exported=Math.max(0,last.grid)*2.4, imported=Math.max(0,-last.grid)*2.2, used=Math.max(0,produced-exported); $('kwh-produced').textContent=produced.toFixed(1); $('kwh-exported').textContent=exported.toFixed(1); $('kwh-imported').textContent=imported.toFixed(1); $('kwh-used').textContent=used.toFixed(1); const self=Math.round(100*Math.max(0,used)/(used+imported||1)); $('self-top').textContent=self+'%'; $('m-self').textContent=self+'%'; drawChart(); }
