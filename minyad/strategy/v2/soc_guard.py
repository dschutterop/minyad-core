...
class SoCGuard:
    ...
    def apply_with_reason(
        ...
        *,
        ...
        return self._apply_soc_hold(setpoint_w, state.battery_soc, self._active_floor(plan, now), plan.effective_soc_ceiling)
    ...
    def _apply_soc_hold(self, setpoint_w: int, soc: float, floor: float, ceiling: float) -> tuple[int, str | None]:
        ...
        if self._charge_blocked:
            ...
            if adjusted != setpoint_w:
                return adjusted, f"guard: SoC ceiling hold ({soc}% >= {ceiling - band}%)"
        return int(setpoint_w), None
