"""Manual override support for strategy v3.

Port of v2's ``OverrideManager`` (unchanged per spec 6: same modes, one-cycle auto-expiry,
``pause`` expiry, ``minyad/control/override`` topic, ``battery_override`` table). The only
adaptation is that v3 has no static ``DayPlan.effective_soc_floor/ceiling`` — the SoC-boundary
checks below reference the tracker's dynamic ``floor_dyn_pct``/``ceil_dyn_pct`` instead, so an
override without ``override_soc_limits`` still respects the same live limits the guard enforces.
"""
...
class OverrideManager:
    ...
    async def apply(self, candidate_w: int, state: ExecutorState, floor_dyn_pct: float, ceil_dyn_pct: float) -> int:
        adjusted, _reason = await self.apply_with_reason(candidate_w, state, floor_dyn_pct, ceil_dyn_pct)
        return adjusted
    ...
    async def apply_with_reason(
        ...
        if mode in {"force_charge", "grid_charge_now"}:
            return self._force_charge(mode, state, ceil_dyn_pct)
        if mode == "force_discharge":
            return self._force_discharge(state, floor_dyn_pct)
        return candidate_w, f"override: unknown mode {mode}"

    def _force_charge(self, mode: str, state: ExecutorState, ceil_dyn_pct: float) -> tuple[int, str]:
        if not self.current.override_soc_limits and state.battery_soc is not None and state.battery_soc >= ceil_dyn_pct:
            return 0, f"override: {mode} blocked at SoC ceiling ({state.battery_soc}% >= {ceil_dyn_pct}%)"
    ...
    def _force_discharge(self, state: ExecutorState, floor_dyn_pct: float) -> tuple[int, str]:
        if not self.current.override_soc_limits and state.battery_soc is not None and state.battery_soc <= floor_dyn_pct:
            return 0, f"override: force_discharge blocked at SoC floor ({state.battery_soc}% <= {floor_dyn_pct}%)"
