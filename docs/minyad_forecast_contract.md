# Minyad ↔ Vesper forecast contract
...
`minyad_forecast` extension to that one response, plus the freshness rules, the quantile
method, and a placeholder design for a future Vesper reservation contract.

## Ownership boundary

Minyad and Vesper are separate projects with a firm split of responsibility:
...
- **Minyad owns:** household energy forecasting, battery dispatch and battery targets, the
  ...
  All of that is produced by the strategy-v3 rolling LP planner (`minyad/strategy/v3/planner.py`,
  see `strategy_v3.md`) and surfaced here.
- **Vesper owns:** appliance timing and device switching. Minyad has no appliance-switching
  logic and must never gain any — `minyad_forecast` is a read-only forecast/budget signal,
  not a command channel. Nothing in this response starts or stops anything.
- **Vesper's job with this response:** subtract its own current-tick Kairos/realtime device
  reservations from `surplus_p50_w`/`surplus_p25_w` itself. Minyad's numbers already exclude
  fixed household base load and Minyad's own planned battery charge/discharge; they do **not**
  know about Vesper's reservations (see [Known-load and reservation semantics](#known-load-and-reservation-semantics)).
...
| `battery.soc_trajectory_pct` | **Compatibility field.** Same 96-value array as `minyad_forecast.soc_pct` below. Vesper currently reads this field for slot-level SoC decisions; it is populated alongside `minyad_forecast.soc_pct` for as long as that's true. Present only when the forecast is valid — never a copy of the current SoC. |

These are the real configured values, not Vesper-style defaults substituted for missing
config — if a value isn't configured, the LP's own default is used (same default the planner
itself uses), never a value invented for this endpoint.
...
- `surplus_p50_w[t]` / `surplus_p25_w[t]` — available incremental household power for
  ...
  battery charge/discharge, clamped to `≥ 0`. They exclude Vesper Kairos/realtime reservations,
  imaginary appliance loads, and double-counted battery power. `surplus_p25_w ≤ surplus_p50_w`
  always (see [Quantile/scenario method](#quantilescenario-method)).
- `pv_p25_w[t]` — the 25th-percentile **PV generation** forecast alone: no household load,
  battery charging, appliance consumption, or Vesper reservations folded in. Used by Vesper's
  battery-first recharge guard. Computed from the PV scenario distribution independently of the
  surplus numbers — it is not derived from `surplus_p25_w`.
...
hold (checked in `minyad/strategy/v3/forecast_contract.py::build_minyad_forecast`):
...
8. A usable PV scenario distribution (persisted `quantile_grid`, see below) exists for every
   slot's cloud-cover class.
...
Minyad does not multiply the point forecast by a fixed discount to get P25 — that would not
be a real uncertainty model. Instead (`minyad/strategy/v3/scenario_forecast.py`):

1. **Historical calibration** (`pv_uncertainty.py`, run daily by the rolling planner): for each
   past PV measurement, compute the ratio of actual to predicted PV output, bucketed by that
   moment's cloud-cover class (`clear` / `partly` / `cloudy`). A class needs ≥ 14 days × 4
   samples/day of history before it's used at all — below that, the class is simply omitted
   (never a fabricated distribution). The result is a compact empirical quantile grid per class
   (9 points: P1, P5, P10, P25, P50, P75, P90, P95, P99), persisted to `pv_uncertainty_bands`.
...
the LP's own solved `soc_target_pct` per slot (`minyad/strategy/v3/models.py::Slot`), never a
Vesper-style synthetic simulation and never a flat copy of the current SoC. Both fields are
populated together during the compatibility period noted above; once Vesper reads
`minyad_forecast.soc_pct` directly, `battery.soc_trajectory_pct` can be retired.
...
profile, see `strategy_v3.md` §3.4). It must **not** include Vesper-controlled appliance jobs
unless there's an explicit, acknowledged reservation — otherwise the same load gets counted
twice (once in Minyad's forecast, once in whatever Vesper adds on top). **No reservation
contract exists in Minyad today**, and none of the `minyad_forecast` fields carry any
reservation/device/acknowledgement data — this is deliberate; an unacknowledged reservation
must never be silently treated as accepted.

### Future reservation contract (design placeholder, not implemented)

If/when Vesper needs to tell Minyad about a planned device dispatch so Minyad can fold it into
`load_forecast_w` without double-counting, that would need its own explicit
...
| `owner` | Which system created it (Vesper, Kairos, etc.). |
| `device_id` / `job_id` | What's being dispatched. |
| `start_time` / `end_time` | Reservation window. |
| `expected_power_curve` | The load Vesper expects to add, per slot. |
...
- `tests/strategy/v3/test_scenario_forecast.py` — scenario sampling, P25 ≤ P50, P25 is not a
  fixed discount of P50, `pv_p25_w` independence, missing-data failure modes.
- `tests/strategy/v3/test_forecast_contract.py` — structural/numeric validation, staleness,
  solver-fallback rejection, slot-count/alignment rejection, no reservation-shaped keys.
- `tests/test_api_status_payloads.py` — `build_surplus_payload()` backward compatibility
  ...
  failure, explicit-UTC timestamps regardless of process timezone.
